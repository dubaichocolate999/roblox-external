...
bool slider_scalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, bool wo_label, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (!temp_input_is_active)
    {
        ...
        const bool clicked = hovered && IsMouseClicked(0, 0);
    ...
    draw->fade_rect_filled(window->DrawList, clickable.Min + ImVec2(2, 2), grab_bb.Max - ImVec2(6, 0), draw->get_clr(clr->accent), draw->get_clr({ clr->accent.Value.x - 0.2f, clr->accent.Value.y - 0.2f, clr->accent.Value.z - 0.2f, 1.f }), fade_direction::vertically, 2.0f);
    ...
    draw->text_clipped(window->DrawList, var->font.tahoma, ImVec2(rect.Max.x - ImGui::CalcTextSize(value_buf).x, rect.Min.y - 1), ImVec2(rect.Max.x, clickable.Min.y - 2), draw->get_clr(clr->widgets.text_inactive), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
