...
bool c_gui::keybind(const char* label, int* key, int* mode)
{
    ...
    draw->rect_filled(window->DrawList, rect.Min + ImVec2(-3, 1), rect.Max - ImVec2(3, 1), draw->get_clr(clr->window.background_two), 2.0f);
    draw->text_clipped(window->DrawList, var->font.tahoma, rect.Min - ImVec2(3, 1), rect.Max - ImVec2(3, 1), draw->get_clr(clr->widgets.text), buf_display, NULL, NULL, ImVec2(0.5f, 0.5f));
    draw->rect(window->DrawList, rect.Min + ImVec2(-3, 0), rect.Max - ImVec2(3, 0), draw->get_clr(var->window.hover_hightlight && hovered ? clr->accent : clr->widgets.stroke_two), 2.0f);
...
{
    draw->text(GetWindowDrawList(), var->font.tahoma, var->font.tahoma->FontSize, GetCursorScreenPos() + ImVec2(1, 0), draw->get_clr(clr->widgets.text), label);
