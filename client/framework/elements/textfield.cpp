...
bool input_text_ex(const char* label, const char* hint, char* buf, int buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        {
            if (Shortcut(ImGuiKey_Tab, ImGuiInputFlags_Repeat))
            {
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const bool is_cut = (Shortcut(ImGuiMod_Ctrl | ImGuiKey_X, f_repeat) || Shortcut(ImGuiMod_Shift | ImGuiKey_Delete, f_repeat)) && !is_readonly && !is_password && (!is_multiline || state->HasSelection());
        const bool is_copy = (Shortcut(ImGuiMod_Ctrl | ImGuiKey_C, 0) || Shortcut(ImGuiMod_Ctrl | ImGuiKey_Insert, 0)) && !is_password && (!is_multiline || state->HasSelection());
        const bool is_paste = (Shortcut(ImGuiMod_Ctrl | ImGuiKey_V, f_repeat) || Shortcut(ImGuiMod_Shift | ImGuiKey_Insert, f_repeat)) && !is_readonly;
        const bool is_undo = (Shortcut(ImGuiMod_Ctrl | ImGuiKey_Z, f_repeat)) && !is_readonly && is_undoable;
        const bool is_redo = (Shortcut(ImGuiMod_Ctrl | ImGuiKey_Y, f_repeat) || (is_osx && Shortcut(ImGuiMod_Ctrl | ImGuiMod_Shift | ImGuiKey_Z, f_repeat))) && !is_readonly && is_undoable;
        const bool is_select_all = Shortcut(ImGuiMod_Ctrl | ImGuiKey_A, 0);

        // We allow validate/cancel with Nav source (gamepad) to makes it easier to undo an accidental NavInput press with no keyboard wired, but otherwise it isn't very useful.
        const bool nav_gamepad_active = (io.ConfigFlags & ImGuiConfigFlags_NavEnableGamepad) != 0 && (io.BackendFlags & ImGuiBackendFlags_HasGamepad) != 0;
        const bool is_enter_pressed = IsKeyPressed(ImGuiKey_Enter, true) || IsKeyPressed(ImGuiKey_KeypadEnter, true);
        const bool is_gamepad_validate = nav_gamepad_active && (IsKeyPressed(ImGuiKey_NavGamepadActivate, false) || IsKeyPressed(ImGuiKey_NavGamepadInput, false));
        const bool is_cancel = Shortcut(ImGuiKey_Escape, f_repeat) || (nav_gamepad_active && Shortcut(ImGuiKey_NavGamepadCancel, f_repeat));
    ...
    if (!is_multiline)
    {
        ...
        if (strlen(buf_display) < 1)
            draw->text_clipped(window->DrawList, var->font.tahoma, rect.Min + ImVec2(10, 0), rect.Max, draw->get_clr(clr->widgets.text_inactive), label, NULL, NULL, ImVec2(0.f, 0.5f));
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
            draw->text(window->DrawList, g.Font, g.FontSize, draw_pos - draw_scroll, draw->get_clr(clr->widgets.text), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
            draw->text(window->DrawList, g.Font, g.FontSize, draw_pos, draw->get_clr(clr->widgets.text), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
    }
