...
void c_draw::text(ImDrawList* draw_list, const ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end, float wrap_width, const ImVec4* cpu_fine_clip_rect)
{
    ...
    font->RenderText(draw_list, font_size, ImVec2(pos.x + 1, pos.y + 1), ImColor(0.1f, 0.1f, 0.1f), clip_rect, text_begin, text_end, wrap_width, cpu_fine_clip_rect != NULL);
    font->RenderText(draw_list, font_size, pos, col, clip_rect, text_begin, text_end, wrap_width, cpu_fine_clip_rect != NULL);
}

void c_draw::text(const char* text)
{
    ImVec2 pos = GetCursorPos();

    gui->push_style_color(ImGuiCol_Text, draw->get_clr({ 0.1f, 0.1f, 0.1f, 1 }));
...
void c_draw::text_clipped(ImDrawList* draw_list, ImFont* font, const ImVec2& pos_min, const ImVec2& pos_max, ImU32 color, const char* text, const char* text_display_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect)
{
    ...
    if (need_clipping)
    {
        ...
        draw_list->AddText(NULL, 0, ImVec2(pos.x + 1, pos.y + 1), ImColor(0.1f, 0.1f, 0.1f), text, text_display_end, 0.0f, &fine_clip_rect);
    }
    else
    {
        draw_list->AddText(NULL, 0, ImVec2(pos.x + 1, pos.y + 1), ImColor(0.1f, 0.1f, 0.1f), text, text_display_end, 0.0f, NULL);
    }
...
{
    draw->rect(GetBackgroundDrawList(), GetWindowPos() - ImVec2(1, 1), GetWindowPos() + GetWindowSize() + ImVec2(1, 1), draw->get_clr({0, 0, 0, 0.5f}), 4.0f);
    draw->rect_filled(GetWindowDrawList(), GetWindowPos() + ImVec2(0, var->window.titlebar), GetWindowPos() + GetWindowSize(), draw->get_clr(clr->window.background_two), 4.0f);
    draw->fade_rect_filled(GetWindowDrawList(), GetWindowPos(), GetWindowPos() + ImVec2(GetWindowWidth(), var->window.titlebar), draw->get_clr(clr->window.background_one), draw->get_clr(clr->window.background_two), fade_direction::vertically, 4.0f);

    draw->rect(GetWindowDrawList(), GetWindowPos(), GetWindowPos() + GetWindowSize(), draw->get_clr(clr->widgets.stroke_two), 4.0f);
    draw->rect_filled(GetWindowDrawList(), GetWindowPos() + ImVec2(1, var->window.titlebar + 3), GetWindowPos() + GetWindowSize() - ImVec2(1, 1) , draw->get_clr(clr->window.background_one), 4.0f);

    draw->text(GetWindowDrawList(), var->font.tahoma, var->font.tahoma->FontSize, GetWindowPos() + ImVec2(7, 4), draw->get_clr(clr->accent), GetCurrentWindow()->Name);
    draw->fade_rect_filled(GetWindowDrawList(), GetWindowPos() + ImVec2(1, var->window.titlebar + 3), GetWindowPos() + ImVec2(GetWindowSize().x - 1, var->window.titlebar + 3 + 1), draw->get_clr(clr->accent), draw->get_clr(clr->window.background_two), fade_direction::horizontally);
}