...
bool c_gui::selectable_ex(const char* label, bool active, const ImVec2& size)
{
    ...
    draw->text_clipped(window->DrawList, var->font.tahoma, rect.Min - ImVec2(0, 1), rect.Max - ImVec2(0, 1), draw->get_clr(active && !hovered ? clr->accent : hovered ? clr->widgets.text_inactive : clr->widgets.text), label, NULL, NULL, ImVec2(0.5f, 0.5f));
