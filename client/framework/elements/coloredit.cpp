...
bool color_button(const char* desc_id, const ImVec4& col)
{
    ...
    RenderColorRectWithAlphaCheckerboard(window->DrawList, rect.Min + ImVec2(3, 2), rect.Max - ImVec2(5, 2), draw->get_clr(col, 0.f), 5, ImVec2(0, 0), 2.0f);
    draw->fade_rect_filled(window->DrawList, rect.Min + ImVec2(3, 2), rect.Max - ImVec2(5, 2), draw->get_clr(col), draw->get_clr({col.x - 0.2f, col.y - 0.2f, col.z - 0.2f, col.w}), fade_direction::vertically, 2.0f);
    draw->rect(window->DrawList, rect.Min + ImVec2(2, 1), rect.Max - ImVec2(4, 1), draw->get_clr(clr->window.stroke), 2.0f);
    return pressed;
}
...
bool color_picker(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    draw->rect(window->DrawList, ImVec2(bar0_pos_x - 1, bar0_line_y), ImVec2(bar0_pos_x + bars_width + 1, bar0_line_y + 3), draw->get_clr(clr->widgets.text), 2.f, 0, 1);
...
{
    draw->text(GetWindowDrawList(), var->font.tahoma, var->font.tahoma->FontSize, GetCursorScreenPos() + ImVec2(1, 0), draw->get_clr(clr->widgets.text_inactive), label.data());
