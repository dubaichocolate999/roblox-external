...
bool c_gui::sub_section(std::string_view label, int section_id, int& section_variable, float count)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect rect(pos, pos + ImVec2(70, elements->section.height));
    ItemSize(rect, style.FramePadding.y);
    if (!ItemAdd(rect, id))
        return false;
    ...
    if (selected || hovered) {
        ...
        draw->line(GetForegroundDrawList(), ImVec2(rect.Min.x + 1, rect.Min.y - 1), ImVec2(rect.Max.x - 1, rect.Min.y - 1), draw->get_clr(clr->accent));
    }
    ...
    draw->text_clipped(window->DrawList, var->font.tahoma, rect.Min, rect.Max, selected ? draw->get_clr(clr->accent) : draw->get_clr(clr->widgets.text_inactive), label.data(), NULL, NULL, ImVec2(0.5f, 0.5f));
