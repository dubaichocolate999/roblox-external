...
bool begin_child_ex(const char* name, ImGuiID id, int x, int y, const ImVec2& size_arg, ImGuiChildFlags child_flags, ImGuiWindowFlags window_flags)
{
    ...
    gui->set_next_window_size(size - ImVec2(0, var->window.titlebar + 3));
    gui->set_next_window_pos(parent_window->DC.CursorPos + ImVec2(0, var->window.titlebar + 3));
    ...
    draw->fade_rect_filled(parent_window->DrawList, parent_window->DC.CursorPos + ImVec2(1, 0), parent_window->DC.CursorPos + ImVec2(size.x - 1, var->window.titlebar + 3), draw->get_clr(clr->window.background_two), draw->get_clr(clr->window.background_one), fade_direction::vertically);
    draw->fade_rect_filled(parent_window->DrawList, parent_window->DC.CursorPos + ImVec2(1, var->window.titlebar + 3), parent_window->DC.CursorPos + ImVec2(size.x - 1 / 2, var->window.titlebar + 3 + 2), draw->get_clr(clr->window.background_two), draw->get_clr(clr->window.background_one), fade_direction::horizontally);
    ...
    draw->text(parent_window->DrawList, var->font.tahoma, var->font.tahoma->FontSize, parent_window->DC.CursorPos + ImVec2(6, 4), draw->get_clr(clr->widgets.text_inactive), name);
