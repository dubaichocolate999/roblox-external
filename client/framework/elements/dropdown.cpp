...
bool dd_selectable_ex(const char* label, bool active)
{
    ...
    draw->text_clipped(window->DrawList, var->font.tahoma, rect.Min + ImVec2(5, -1), rect.Max - ImVec2(0, 1), draw->get_clr(active && !hovered ? clr->accent : hovered ? clr->widgets.text_inactive : clr->widgets.text), label, NULL, NULL, ImVec2(0.f, 0.5f));
...
bool begin_dropdown_popup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags, int val)
{
    ...
    draw->rect(GetWindowDrawList(), GetWindowPos() + ImVec2(1, 1), GetWindowPos() + GetWindowSize() - ImVec2(1, 1), draw->get_clr(clr->window.stroke), 2.0f);
    draw->rect_filled(GetWindowDrawList(), GetWindowPos() + ImVec2(2, 2), GetWindowPos() + GetWindowSize() - ImVec2(2, 2), draw->get_clr(clr->window.background_one), 2.0f);
...
bool begin_dropdown(const char* label, const char* preview_value, bool wo_label, int val, ImGuiComboFlags flags)
{
    ...
    // Render shape
    draw->fade_rect_filled(window->DrawList, clickable.Min + ImVec2(2, 2), clickable.Max - ImVec2(2, 2), draw->get_clr(clr->window.background_two), draw->get_clr(clr->window.background_one), fade_direction::vertically, 2.0f);
    draw->rect(window->DrawList, clickable.Min + ImVec2(1, 1), clickable.Max - ImVec2(1, 1), draw->get_clr(clr->window.stroke), 2.0f);

    draw->text_clipped(window->DrawList, var->font.tahoma, clickable.Min + ImVec2(5, 0), clickable.Max - ImVec2(20, 0), draw->get_clr(clr->widgets.text_inactive), preview_value, NULL, NULL, ImVec2(0.f, 0.5f));

    if (!wo_label)
        draw->text_clipped(window->DrawList, var->font.tahoma, rect.Min - ImVec2(-1, 2), ImVec2(rect.Max.x, clickable.Min.y - 2), draw->get_clr(clr->widgets.text_inactive), label, NULL, NULL, ImVec2(0.f, 0.f));
